...
class UserRightsList(APIView):
    def get(self, request):
        userrights = UserRights.objects.all()
        serializer = UserRightsSerializer(userrights, many=True)
        return Response(serializer.data)
    ...
    def post(self, request):
        try:
            ...
            try:
                rightid = request.POST['rightid']
                right = Rights.objects.filter(id=rightid)
                if len(right) == 1:
                    right = right[0]
                    if right.is_active == False:
                        res = {
                            'status': 403,
                            'error': 'Right is Inactive'
                        }
                        return JsonResponse(res)
                else:
                    res = {
                        'status': 404,
                        'error': 'Right is not present'
                    }
                    return JsonResponse(res)
            except Exception as e:
                right = None
            try:
                ...
                group = RightGroups.objects.filter(id=groupid)
                if len(group) == 1:
                    group = group[0]
            ...
            if group is not None:
                res['group'] = group.id
            res['message'] = 'User Right/Right Group added successfully'
    ...
    def delete(self, request):
        try:
            id = request.POST['id']
            userright = UserRights.objects.get(id=id)
            userright.delete()
            res = {
                'status': 200,
                'message': 'User Right Removed Successfully'
            }
        except Exception as e:
            res = {
                'status': 500,
